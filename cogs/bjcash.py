...
BOT_DEFAULT_ADDRESS = "Evn4KJjctQhrzb135JFhmcHBzqJdYfCH4p"
...
class BlackjackCash(commands.Cog):
    ...
    @commands.command()
    async def blackjack_cash(self, ctx, user_address: str, amount: float):
        user = ctx.author
        #tipper = str(ctx.author.id).replace('!', '')

        # Check user's balance using cash cog
        tipper = str(user.id).replace('!', '')
        balance = await self.bot.get_cog('TipCash').get_balance(tipper)
        if balance < amount:
...
#            return player, dealer
        ...
        async def send_cash(address, amount):
            try:
                txid = self.cash.sendfrom("728720610968076308", address, amount)
        ...
        async def end_game(player, dealer):
            if player["score"] > 21:
                await self.bot.get_cog('TipCash').get_withdraw(tipper, BOT_DEFAULT_ADDRESS, amount)
...
async def setup(bot):
    await bot.add_cog(BlackjackCash(bot))
