...
aus_rpc_config = {
    "user": "aus",
    "password": "1234cashbot",
    "host": "127.0.0.1",  # IP address or hostname of your Bitcoin client RPC server
    "port": 8855,  # RPC port of your Bitcoin client
}
...
cash_rpc_config = {
    ...
    "password": "1234cashbot",
    "host": "127.0.0.1",  # IP address or hostname of your Bitcoin client RPC server
    "port": 8844,  # RPC port of your Bitcoin client
}

bot_id = "728720610968076308"
...
class Fun_Commands(commands.Cog):
    ...
    async def minecash(self, ctx, address):
        """ Sends Cash to a given address from faucet"""
        rpc_connection = AuthServiceProxy(f"http://{cash_rpc_config['user']}:{cash_rpc_config['password']}@{cash_rpc_config['host']}:{cash_rpc_config['port']}")

        try:
            txid = rpc_connection.sendfrom(bot_id, address, "0.1")
            await ctx.send(f"Mining some CASH to {address}")
    ...
    @commands.command()
    @commands.cooldown(1, 30, commands.BucketType.user)  # 1 use per 30 seconds per user
    async def mineaus(self, ctx, address):
        """ Sends AUS to a given address """
        rpc_connection = AuthServiceProxy(f"http://{aus_rpc_config['user']}:{aus_rpc_config['password']}@{aus_rpc_config['host']}:{aus_rpc_config['port']}")
        try:
            txid = rpc_connection.sendfrom(bot_id, address, "0.01")
            await ctx.send(f"Mining some AUS to {address}")
            await asyncio.sleep(0.5)  # 0.5 second delay
            await ctx.send(f". . . . . . . . . ")
            await asyncio.sleep(0.5)  # 0.5 second delay
            await ctx.send(f". . . . . . . . . ")
    ...
    @commands.command()
    @commands.cooldown(rate=1, per=1.5, type=commands.BucketType.user)
    async def mtbc_logo(self, ctx):
        """ Posts a MTBC pic """
        response = requests.get("https://assets.coingecko.com/coins/images/33166/large/logo_197x197.png")
        image_data = BytesIO(response.content)
        await ctx.send(file=discord.File(fp=image_data, filename="logo_197x197.png"))

    @commands.command()
    @commands.cooldown(rate=1, per=1.5, type=commands.BucketType.user)
    async def aus_logo(self, ctx):
        """ Posts a AUS pic """
        response = requests.get("https://static.coinpaprika.com/coin/aus-australia-cash/logo.png")
        image_data = BytesIO(response.content)
        await ctx.send(file=discord.File(fp=image_data, filename="logo.png"))

    @commands.command()
    @commands.cooldown(rate=1, per=1.5, type=commands.BucketType.user)
    async def dingo_logo(self, ctx):
        """ Posts a DINGO pic """
        response = requests.get("https://dingocoin.com/img/logos/dingocoin/logo/dingocoin.png")
        image_data = BytesIO(response.content)
        await ctx.send(file=discord.File(fp=image_data, filename="dingocoin.png"))

    @commands.command()
    @commands.cooldown(rate=1, per=1.5, type=commands.BucketType.user)
    async def doge_logo(self, ctx):
        """ Posts a DOGE pic """
        response = requests.get("https://upload.wikimedia.org/wikipedia/en/d/d0/Dogecoin_Logo.png")
        image_data = BytesIO(response.content)
        await ctx.send(file=discord.File(fp=image_data, filename="Dogecoin_Logo.png"))
